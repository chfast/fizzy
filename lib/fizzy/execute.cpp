...
inline bool invoke_function(
    const FuncType& func_type, const F& func, Instance& instance, Value*& sp, int depth) noexcept
{
    const auto num_args = func_type.inputs.size();
    //    assert(stack.size() >= num_args);
    span<const Value> call_args{sp + 1 - num_args, num_args};
...
ExecutionResult execute(Instance& instance, FuncIdx func_idx, const Value* args, int depth) noexcept
{
    ...
    while (true)
    {
        ...
        case Instr::f32_abs:
        {
            ...
            unary_op(sp, static_cast<float (*)(float)>(std::fabs));
            break;
        }
        case Instr::f32_neg:
        {
            unary_op(sp, std::negate<float>{});
            break;
        }
        case Instr::f32_ceil:
        {
        ...
        case Instr::f32_copysign:
        {
            ...
            binary_op(sp, static_cast<float (*)(float, float)>(std::copysign));
            break;
        }
        ...
        {
            // TODO: This can be optimized https://godbolt.org/z/aPqvfo
            unary_op(sp, static_cast<double (*)(double)>(std::fabs));
            break;
        }
        case Instr::f64_neg:
        {
            unary_op(sp, std::negate<double>{});
            break;
        }
        case Instr::f64_ceil:
        {
        ...
        {
            binary_op(sp, static_cast<double (*)(double, double)>(std::copysign));
            break;
        }
