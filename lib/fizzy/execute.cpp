...
ExecutionResult execute(Instance& instance, FuncIdx func_idx, const Value* args, int depth) noexcept
{
    ...
    while (true)
    {
        ...
        case Instr::f32_abs:
        {
            ...
            unary_op(&stack.top(), static_cast<float (*)(float)>(std::fabs));
            break;
        }
        case Instr::f32_neg:
        {
            unary_op(&stack.top(), std::negate<float>{});
            break;
        }
        case Instr::f32_ceil:
        {
        ...
        {
            // TODO: This can be optimized https://godbolt.org/z/aPqvfo
            unary_op(&stack.top(), static_cast<double (*)(double)>(std::fabs));
            break;
        }
        case Instr::f64_neg:
        {
            unary_op(&stack.top(), std::negate<double>{});
            break;
        }
        case Instr::f64_ceil:
        {
